...
static BOLT4_SESSION_KEY: &'static str = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";

// BOLT4_ASSOC_DATA is the associated data added to the packet.
static BOLT4_ASSOC_DATA: &'static str = "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB";

// bolt4_final_packet_hex encodes the expected sphinx packet as a result of
// creating a new packet with the above parameters.
// TODO(vlad)
fn bolt4_final_packet_hex() -> String {
    "0002eec7245d6b7d2ccb30380bfbe2a3648cd7".to_owned() +
...
fn test_bolt4_packet() {
    ...
    let path = BOLT4_PUB_KEYS.iter().enumerate().map(|(i, &d)| {
    ...
    let session_key = SecretKey::from_slice(&Secp256k1::new(), BOLT4_SESSION_KEY.as_bytes()).unwrap();
    let route = OnionRoute::new(OnionPacketVersion::_0, session_key, path, BOLT4_ASSOC_DATA.as_bytes().to_vec());

    let packet = route.packet().unwrap();
    let reference_packet = hex::decode(bolt4_final_packet_hex())
