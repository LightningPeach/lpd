...
static HANDSHAKE_READ_TIMEOUT: u8 = 5;
...
impl Machine {
    ...
    fn gen_act_one(&mut self) -> Result<[u8; ACT_ONE_SIZE], HandshakeError> {
    	...
    	act_one[34..].copy_from_slice(&auth_payload);
    ...
    fn recv_act_one(&mut self, act_one: [u8; ACT_ONE_SIZE]) -> Result<(), HandshakeError> {
        ...
        let mut p: [u8; MAC_SIZE] = [0; MAC_SIZE];

        e.copy_from_slice(&act_one[1..34]);
    ...
    fn gen_act_two(&mut self) -> Result<[u8; ACT_TWO_SIZE], HandshakeError> {
    	...
    	act_two[34..].copy_from_slice(&auth_payload);
    ...
    fn gen_act_three(&mut self) -> Result<[u8; ACT_THREE_SIZE], HandshakeError> {
        ...
        act_three[1..34].copy_from_slice(ciphertext.as_slice());
        act_three[34..50].copy_from_slice(&tag);
        act_three[50..].copy_from_slice(&auth_payload);
    ...
    fn recv_act_three(&mut self, act_three: [u8; ACT_THREE_SIZE]) -> Result<(), HandshakeError> {
        ...
        // let mut s: [u8; 33 + 16] = [0; 33 + 16];
        let mut p: [u8; MAC_SIZE] = [0; MAC_SIZE];

        ciphertext.copy_from_slice(&act_three[1..33+1]);
        tag.copy_from_slice(&act_three[33+1..33+16+1]);
        // s.copy_from_slice(&act_three[1..33+16+1]);
