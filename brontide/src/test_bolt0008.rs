...
fn test_bolt0008_internal() -> Result<(), Box<Error>> {
    let rs_priv = SecretKey::from_slice(
        &Secp256k1::new(), hex::decode("2121212121212121212121212121212121212121212121212121212121212121")?.as_slice())?;
    let rs_pub = PublicKey::from_secret_key(&Secp256k1::new(), &rs_priv)?;
    assert_eq!(hex::encode(&rs_pub.serialize()[..]), "028d7500dd4c12685d1f568b4c2b5048e8534b873319f3a8daa612b469132ec7f7");

    let ls_priv = SecretKey::from_slice(
        &Secp256k1::new(), hex::decode("1111111111111111111111111111111111111111111111111111111111111111")?.as_slice())?;
    let ls_pub = PublicKey::from_secret_key(&Secp256k1::new(), &ls_priv)?;
    assert_eq!(hex::encode(&ls_pub.serialize()[..]), "034f355bdcb7cc0af728ef3cceb9615d90684bb5b2ca5f859ab0f0b704075871aa");
    ...
    machine.ephemeral_gen = || -> Result<SecretKey, Box<Error>> {
		let sk = SecretKey::from_slice(
            &Secp256k1::new(),hex::decode("1212121212121212121212121212121212121212121212121212121212121212")?.as_slice())?;
        Ok(sk)
	};
    assert_eq!(hex::encode(machine.handshake_state.symmetric_state.handshake_digest), "8401b3fdcaaa710b5405400536a3d5fd7792fe8e7fe29cd8b687216fe323ecbd");
    ...
    responder_machine.ephemeral_gen = || -> Result<SecretKey, Box<Error>> {
		let sk = SecretKey::from_slice(
            &Secp256k1::new(),hex::decode("2222222222222222222222222222222222222222222222222222222222222222")?.as_slice())?;
        Ok(sk)
	};
